...
void *philosopher_thread(void *argument) {
  ...
  while (again) {
    ...
    if (sem_trywait(&forks[philosopher->leftForkIndex].mutex) == 0) {
      ...
      int waiting_times =
          10 + rand() % 50; /* returns number in [10..59] interval */
       ...
       * waiting time is expired - he will release left fork, despite he is
       * hungry */
...
int main(int argc, char *argv[]) {
  ...
  philosophers =
      (struct Philosopher *)malloc(sizeof(struct Philosopher) * count);

  /* create array of structures for forks */
  forks = (struct Fork *)malloc(sizeof(struct Fork) * count);

  /* create global mutex in order to determinate all philophers eaten at least 1
   * time */
